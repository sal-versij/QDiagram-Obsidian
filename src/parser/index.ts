...
import { BUILTIN_GATES, getBuiltinGateArity } from "../shared/gate-registry";
...
import {
  AliasDecl,
  ClassicalAliasDecl,
  resolveClassicalBitsAndAliases,
  resolveQubitsAndAliases
...
function parseAliasDecl(statement: string, line: number): AliasDecl | ClassicalAliasDecl {
  const match = statement.match(/^alias\s+([qc])(\d+)\s*=\s*([A-Za-z_][A-Za-z0-9_]*)$/i);
  if (!match) {
  ...
  return {
    line,
    index: parsePositiveInt(match[2], line, "alias index"),
    name: match[3]
  };
}

function parseInlineAliasList(aliasSource: string, line: number, kind: "q" | "c"): Array<AliasDecl | ClassicalAliasDecl> {
  const specs = aliasSource
    .split(",")
    .map((entry) => entry.trim())
    .filter((entry) => entry.length > 0);

  const aliases: Array<AliasDecl | ClassicalAliasDecl> = [];
  const regex = kind === "q" ? /^q(\d+)\s*=\s*([A-Za-z_][A-Za-z0-9_]*)$/i : /^c(\d+)\s*=\s*([A-Za-z_][A-Za-z0-9_]*)$/i;
...
function parseOperation(statement: string, line: number): TempOp {
  const measureMatch = statement.match(/^(MEASURE|M)\s+([^\s]+)\s*->\s*([^\s]+)$/i);
  if (measureMatch) {
    return {
      kind: "measure",
      line,
      targetRef: measureMatch[2],
      classicalTargetRef: measureMatch[3]
    };
  }
  ...
  let conditionalRef: string | undefined;
  const conditionalMatch = statement.match(/\[\s*([A-Za-z_][A-Za-z0-9_]*|c\d+)\s*\]\s*$/i);
  if (conditionalMatch) {
    conditionalRef = conditionalMatch[1];
  ...
  if (arity !== undefined) {
    ...
    return { kind: "gate", line, name: gate, params, targetRefs: argRefs, conditionalRef };
  }
...
function resolveClassicalRef(
  ref: string,
  aliasesByName: Map<string, number>,
  classicalBits: number,
  line: number
): number {
  if (/^\d+$/.test(ref)) {
    const target = parsePositiveInt(ref, line, "classical bit index");
    if (target >= classicalBits) {
      throw new Error(`Line ${line}: target c${target} is out of range for ${classicalBits} classical bits.`);
    }
    return target;
  }

  const cStyle = ref.match(/^c(\d+)$/i);
  if (cStyle) {
    const target = parsePositiveInt(cStyle[1], line, "classical bit index");
    if (target >= classicalBits) {
      throw new Error(`Line ${line}: target c${target} is out of range for ${classicalBits} classical bits.`);
    }
    return target;
  ...
  const aliasTarget = aliasesByName.get(ref);
  if (aliasTarget === undefined) {
    throw new Error(`Line ${line}: unknown classical bit reference '${ref}'.`);
  }
  return aliasTarget;
}
...
class CircuitBuilder {
  ...
  build(): CircuitAst {
    ...
    let classicalBits: number | undefined;

    const qubitAliasDecls: AliasDecl[] = [];
    const classicalAliasDecls: ClassicalAliasDecl[] = [];
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      ...
      if (inBraces && /^\s*(qubits|cbits|alias)\b/i.test(statement)) {
      ...
      if (qubitsMatch) {
        ...
        if (inlineAliasSource.length > 0) {
          const aliases = parseInlineAliasList(inlineAliasSource, token.line, "q");
          qubitAliasDecls.push(...aliases as AliasDecl[]);
        }
      ...
      const cbitsMatch = statement.match(/^cbits\s+(\d+)(?:\s*:\s*(.+))?$/i);
      if (cbitsMatch) {
        if (inBraces || commaGroupId !== undefined) {
          throw new Error(`Line ${token.line}: 'cbits' cannot appear inside an operation group.`);
        }
        if (classicalBits !== undefined) {
          throw new Error(`Line ${token.line}: cbits already declared.`);
        }
        const count = parsePositiveInt(cbitsMatch[1], token.line, "classical bit count");
        if (count <= 0) {
          throw new Error(`Line ${token.line}: cbits must be greater than zero.`);
        }
        classicalBits = count;

        let inlineAliasSource = cbitsMatch[2] ? cbitsMatch[2].trim() : "";
        let consumeIndex = i + 1;
        while (
          consumeIndex + 1 < tokens.length &&
          tokens[consumeIndex].type === "comma" &&
        ...
        if (inlineAliasSource.length > 0) {
          const aliases = parseInlineAliasList(inlineAliasSource, token.line, "c");
          classicalAliasDecls.push(...aliases as ClassicalAliasDecl[]);
        }
      ...
      if (/^alias\b/i.test(statement)) {
        ...
        const parsedAlias = parseAliasDecl(statement, token.line);
        if (/^alias\s+q/i.test(statement)) {
          qubitAliasDecls.push(parsedAlias as AliasDecl);
        } else if (/^alias\s+c/i.test(statement)) {
          classicalAliasDecls.push(parsedAlias as ClassicalAliasDecl);
        } else {
          throw new Error(`Line ${token.line}: alias must target qN or cN.`);
        }
        continue;
      }
    ...
    const { resolvedQubits, aliasByIndex, aliasByName } = resolveQubitsAndAliases({
      qubits,
      parsedItems,
      aliasDecls: qubitAliasDecls,
      customGateNames: new Set(gateDecls.keys())
    });

    const {
      resolvedClassicalBits,
      aliasByIndex: classicalAliasByIndex,
      aliasByName: classicalAliasByName
    } = resolveClassicalBitsAndAliases({
      classicalBits,
      parsedItems,
      aliasDecls: classicalAliasDecls,
      qubitAliasByName: aliasByName
    });
    ...
    for (const item of parsedItems) {
      const op = item.op;
      if (op.kind === "gate") {
        const conditionalBit = op.conditionalRef
          ? resolveClassicalRef(op.conditionalRef, classicalAliasByName, resolvedClassicalBits, item.line)
          : undefined;

        if (BUILTIN_GATES.has(op.name)) {
          const targets = op.targetRefs.map((ref) => resolveQubitRef(ref, aliasByName, resolvedQubits, item.line));
          resolvedOps.push({
            line: item.line,
    ...
    const ast: CircuitAst = {
      ...
      classicalBits: resolvedClassicalBits,
      phases,
      ops
    };
    ...
    if (classicalAliasByIndex.size > 0) {
      ast.classicalAliases = classicalAliasByIndex;
    }
